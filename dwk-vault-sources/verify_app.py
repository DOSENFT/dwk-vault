...
time.sleep(1.5)
try:
    man = json.load(urllib.request.urlopen(URL + "manifest.webmanifest"))
    ...
    for i in man['icons']:
        check(urllib.request.urlopen(URL + i['src']).status == 200, f"icon {i['src']} is actually there")
    check(urllib.request.urlopen(URL + "apple-touch-icon.png").status == 200, "the iOS icon is there")
