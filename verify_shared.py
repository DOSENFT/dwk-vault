...
with sync_playwright() as b0:
    ...
    check(pg.eval_on_selector('#e-hero', "e=>e.classList.contains('on')"), "a hero shot lands behind his name")
    ...
    pg.evaluate("openE('Brannoc')"); pg.wait_for_timeout(400)
    ...
    picks = pg.eval_on_selector_all('.shpick button', 'e=>e.map(x=>x.textContent)')
    check(sorted(picks) == sorted(pg.evaluate("CAMPAIGN.nations")),
          f"it offers the five real nations rather than a blank box ({picks})")
    pg.click('.shvis button[data-v="all"]'); pg.wait_for_timeout(200)
    pg.click('.shpick button'); pg.wait_for_timeout(200)
    check(pg.input_value('#sh-t') == picks[0], "tapping one starts the line for him")
    pg.fill('#sh-t', 'A fishing town the maps stopped printing.')
    check(pg.eval_on_selector('#sh-t', 'e=>e.value') == 'A fishing town the maps stopped printing.',
          "and he can throw it away and write his own")
    pg.click('#sh-ok'); pg.wait_for_timeout(600)
    pg.evaluate("void sayEdit('Brannoc','heard')"); pg.wait_for_timeout(400)
    heard = pg.eval_on_selector_all('.shpick button', 'e=>e.map(x=>x.textContent)')
    check(len(heard) >= 6 and all(pg.evaluate("n=>!!codexIndex()[n.toLowerCase()]", h) for h in heard),
          f"and 'who have you heard of' offers real names out of the record ({heard[:4]})")
    pg.click('#sh-x'); pg.wait_for_timeout(300)
    check(pg.evaluate("(sayOf('Brannoc').from||{}).v") == 'A fishing town the maps stopped printing.',
          "it is written down")
    check(pg.eval_on_selector_all('.sayrow', 'e=>e.length') == 3, "all three questions are on the page")
    ...
    n = pg2.eval_on_selector_all('.kitem', 'e=>e.length')
    check(n == 2, f"two items on his screen, not three ({n})")
